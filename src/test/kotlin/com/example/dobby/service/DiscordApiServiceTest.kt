...
class DiscordApiServiceTest {
    @Test
    fun `exchangeCodeForToken should return response on valid status`() = runTest {
    ...
    @Test
    fun `exchangeCodeForToken should throw AuthorizationException when Discord returns HTTP error`() = runTest {
        val mockEngine = MockEngine { _ ->
            respond(
    ...
    @Test
    fun `getUserProfile should throw AuthorizationException when token is expired or invalid`() = runTest {
        val mockEngine = MockEngine { _ ->
            respond(
    ...
    @Test
    fun `fetchCompleteUserProfile should filter guilds using bitwise admin flags`() = runTest {
    ...
    @Test
    fun `fetchCompleteUserProfile should use default embed avatar when avatar hash is null`() = runTest {
        val mockEngine = MockEngine { request ->
            when (request.url.encodedPath) {
                "/api/v10/users/@me" -> respond(
                ...
                "/api/v10/users/@me/guilds" -> respond(
                    content = "[]",
                    status = HttpStatusCode.OK,
                    headers = headersOf(HttpHeaders.ContentType, "application/json")
                )
