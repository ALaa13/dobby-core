        ...
        runTest {
            ...
            coVerify(exactly = 1) {
                userProfileRepository.saveProfile(
                    match {
                        it.discordUserId == "user-123" &&
                            it.guildId == "guild-456" &&
                            it.displayName == "SomeUser" &&
                            it.avatarHash == "00112233"
                    },
                )
            }
            coVerify(exactly = 1) {
                userFactRepository.saveFact(
                    match {
                        it.profileId == "1" &&
                            it.factText == "He'll fix a kernel panic before fixing his posture." &&
                            it.source == FactSource.USER_SUBMISSION
                    },
                )
            }
        }
